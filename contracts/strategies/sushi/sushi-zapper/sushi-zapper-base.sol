...
abstract contract ZapperBase {
  ...
  function zapInETH(
    ...
    uint256 tokenAmountOutMin,
    address tokenIn
  ) external payable onlyWhitelistedVaults(vault_addr) returns (uint256 vaultBalance) {
    ...
    if (tokenIn != weth) {
      ...
      vaultBalance = _swapAndStake(vault_addr, tokenAmountOutMin, tokenIn);
    } else {
      vaultBalance = _swapAndStake(vault_addr, tokenAmountOutMin, tokenIn);
    }
  }
  ...
  function zapIn(
  ...
  ) external onlyWhitelistedVaults(vault_addr) returns (uint256 vaultBalance) {
    ...
    if (tokenIn != pair.token0() && tokenIn != pair.token1()) {
      ...
      vaultBalance = _swapAndStake(vault_addr, tokenAmountOutMin, desiredToken);
    } else {
      vaultBalance = _swapAndStake(vault_addr, tokenAmountOutMin, tokenIn);
    }
  }
