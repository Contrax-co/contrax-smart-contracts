...
abstract contract StrategyEthStargateFarmBase is StrategyBase {
    ...
    address public ethRouter = 0xbf22f0f184bCcbeA268dF387a49fF5238dD23E40;
    ...
    function harvest() public override onlyBenevolent sphereXGuardPublic(0x22c6abe5, 0x4641257d) {
        ...
        console.log("the amount of weth is: ", _wantBase);
        WETH(weth).withdrawTo(address(this), _wantBase);

        _wantBase = IERC20(baseToken).balanceOf(address(this));
        console.log("the amount of weth is: ", _wantBase);

        IEthRouter(ethRouter).addLiquidityETH();
