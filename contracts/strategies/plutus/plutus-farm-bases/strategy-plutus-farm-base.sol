...
abstract contract StrategyPlutusPlsDpxFarmBase is StrategyBase {
  ...
  address public constant plsJones = 0xe7f6C3c1F0018E4C08aCC52965e5cbfF99e34A44;
  ...
  function harvest() public override onlyBenevolent sphereXGuardPublic(0xc3c63ae8, 0x4641257d) {
    ...
    uint256 _plsJones = IERC20(plsJones).balanceOf(address(this));
    if(_plsJones > 0){
      // 10% is locked up for future gov
      uint256 _keep = _plsJones.mul(keep).div(keepMax);
      if (_keep > 0) {
          IERC20(plsJones).safeTransfer(
                IController(controller).treasury(),
                _keep
          );
      }
      _plsJones = IERC20(plsJones).balanceOf(address(this));

      if(want != plsJones) {
        _swapSushiswap(plsJones, dpx, _plsJones);
      }
    }
