...
contract VaultLPZapperPeapods is PeapodsLPZapperBase {
    ...
    function _swapAndStake(
        ...
        if (tokenIn != weth && tokens[1] != weth) {
            ...
            console.log("i think this is before the problem");

            //_swapCamelotWithPath(path, fullInvestment.div(2));

            console.log("i think this is immediately after the problem");
        } else {
        ...
        IDecentralizedIndex(indexUtils).bond(
            apToken[tokens[0]],
            tokens[0],
            _want0,
            0
