...
contract StrategyComethFarmTestBase is DSTestDefiBase {
    address want;
    address token1;
    ...
    function _getWant(uint256 wethAmount, uint256 token1Amount) internal {
        _getERC20(weth, wethAmount);
        _getERC20(token1, token1Amount);

        uint256 _wethAmount = IERC20(weth).balanceOf(address(this));
        uint256 _token1 = IERC20(token1).balanceOf(address(this));

        IERC20(weth).safeApprove(address(univ2), 0);
        IERC20(weth).safeApprove(address(univ2), _wethAmount);
