...
import { setupSigners, reward_addr, treasury_addr } from './utils/static';
import {
...
export function doStrategyTest(test_case: TestableStrategy) {
  ...
  describe('Tests for: ' + name, async () => {
    ...
    before(async () => {
      ...
      assetContract = await ethers.getContractAt(
        'ERC20',
        asset_addr,
        walletSigner
      );
    ...
    const harvester = async () => {
      await overwriteTokenAmount(asset_addr, wallet_addr, txnAmt, slot);
      let amt = await assetContract
        .connect(walletSigner)
        .balanceOf(wallet_addr);

      let balBefore = await assetContract
        .connect(walletSigner)
      ...
      let userBal = await assetContract
        .connect(walletSigner)
        .balanceOf(wallet_addr);
      expect(userBal).to.be.equals(BigNumber.from('0x0'));

      let balAfter = await assetContract
        .connect(walletSigner)
        .balanceOf(vault_addr);
      ...
      let harvestable_function = Strategy.functions['getHarvestable']
        ? 'getHarvestable'
        : 'getWavaxAccrued';
    ...
    it('user wallet contains asset balance', async function () {
      let BNBal = await assetContract.balanceOf(
    ...
    if (test_case.type != 'FOLD') {
      it('Controller strategy to be configured correctly', async () => {
        expect(await Controller.strategies(asset_addr)).to.be.equals(
          strategy_addr
        );
      });
    ...
    it('Users should earn some money!', async function () {
      ...
      await Vault.connect(walletSigner).withdrawAll();
      let newAmt = await assetContract
        .connect(walletSigner)
        .balanceOf(wallet_addr);
    ...
    it('should take no commission when fees not set', async function () {
      ...
      let rewardContract = await ethers.getContractAt(
        'ERC20',
        reward_addr,
        walletSigner
      );
      ...
      const treasuryBefore = await assetContract
        .connect(walletSigner)
        .balanceOf(treasury_addr);
      ...
      const treasuryAfter = await assetContract
        .connect(walletSigner)
        .balanceOf(treasury_addr);
      ...
      console.log(
        `\t💸Treasury profit after harvest:  ${earntTreasury.toString()}`
      );
