...
let name = "VaultSteerSushiUsdcUsdce";
let steer_vault_addr = "0x3eE813a6fCa2AaCAF0b7C72428fC5BC031B9BD65";
let sushiV3Router = "0xE592427A0AEce92De3Edee1F18E0157C05861564"
...
let asset_addr_weth = "0x82aF49447D8a07e3bd95BD0d56f35241523fBab1";
...
describe( "Tests for Zapper", async () => {
    ...
    before(async () => {
        ...
        const vaultName = `LocalVault${name}`;
        ...
        Zapper = await zapperFactory.connect(walletSigner).deploy(sushiV3Router,walletSigner.getAddress());

        const localVaultFactory = await ethers.getContractFactory('VaultSteerSushiUsdcUsdce');
        LocalVault = await localVaultFactory.connect(walletSigner).deploy(steer_vault_addr,walletSigner.getAddress(),walletSigner.getAddress());

  
        zapper_addr = Zapper.address;
        console.log(`Deployed Zapper at ${zapper_addr}`);

        localVault_addr = LocalVault.address;
        console.log(`Deployed LocalVault at ${localVault_addr}`);
        
        //whitelist vault address in zapper contract
        await Zapper.connect(walletSigner).addToWhitelist(localVault_addr);

        //Set local vault address against steer vault
        await Zapper.connect(walletSigner).addSteerVault(localVault_addr,steer_vault_addr);
    ...
    const zapInETH = async () => {
      let _vaultBefore = await LocalVault.connect(walletSigner).balanceOf(await walletSigner.getAddress()); 
      await Zapper.connect(walletSigner).zapInETH(localVault_addr, 0, asset_addr_weth, {value: "15000000000000000000"});

      let _vaultAfter = await LocalVault.connect(walletSigner).balanceOf(await walletSigner.getAddress()); 
    ...
    const zapIn = async () => {
      ...
      await Zapper.connect(walletSigner).zapIn(localVault_addr, 0, asset_addr2, txnAmt);
    ...
    it("Should withdraw from the vault and zap into the native tokens", async function() {
      ...
      await LocalVault.connect(walletSigner).approve(zapper_addr, _vaultAfter);
      await Zapper.connect(walletSigner).zapOutAndSwapEth(localVault_addr, _vaultAfter, 0);

      _vaultAfter = await LocalVault.connect(walletSigner).balanceOf(wallet_addr);
      ...
      expect(Number(_balDifference)).to.be.gt(0.9);
    }); 
    ...
    it("Should withdraw from vault andd zap into usdc", async function () {
      ...
      await LocalVault.connect(walletSigner).approve(zapper_addr, _vaultAfter);
      await Zapper.connect(walletSigner).zapOutAndSwap(localVault_addr, _vaultAfter, asset_addr2, 0);
      ...
      expect(_vaultAfter).to.be.equals(BigNumber.from("0x0"));
      expect(Number(_balDifference)).to.be.gt(0.4);
