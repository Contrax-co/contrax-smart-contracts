...
let name = "SushiWethWbtc";
let vault_addr = "0xdf9d86bC4765a9C64e85323A9408dbee0115d22E";
...
describe( "Tests for Zapper", async () => {
    ...
    const zapIn = async () => {
      ...
      let _vaultAfter = await Vault.connect(walletSigner).balanceOf(await walletSigner.getAddress());
      console.log("the balance in the vault after depositing is", _vaultAfter);

      let assetBalAfter2 = await assetContract2.connect(walletSigner).balanceOf(await walletSigner.getAddress());
      console.log("the balance of the asset after depositing is", assetBalAfter2)
