...
let packedInput = "36537540934274697395364984131296603634800490339341454694449";
let packedConfig = "2890998072125432283472529353134770243007992687017183936539";
let r = "0x2472e9159cf2f7c25249ca70d1720c5ee772695fc502f9ba78f2d2f9cbb6a054";
let s = "0x47119127ef9c235184d8fe0ae4a02df257ca3cdbaaee921cd484c63286b62972";

const vaultName = "VaultClipperBase";
const strategyName = "StrategyClipper";

describe("Clipper Zapper Test", async () => {
  ...
  before(async () => {
    ...
    const controllerFactory = await ethers.getContractFactory("ClipperController");
    ...
    vaultContract = await vaultFactory
      .connect(walletSigner)
      .deploy(clipperAddress, walletSigner.getAddress(), walletSigner.getAddress(), controllerAdd);
    ...
    const approveStrategy = await controllerContract
      ...
      .approveStrategy(clipperAddress, startegyContract.address);
    ...
    await setStrategy(strategyName, controllerContract, timelockSigner, clipperAddress, startegyContract.address);

    const zapperFactory = await ethers.getContractFactory("ClipperZapperBase");
    ...
    // await overwriteTokenAmount(clipperAddress, startegyContract.address, strategySteerVaultAmount, 9);
  });
  ...
  const zapInETH = async () => {
    ...
    await zapperContract.connect(walletSigner).zapInETH(vaultContract.address, 0, packedInput, packedConfig, r, s, {
  ...
  const zapIn = async () => {
    ...
    await zapperContract.connect(walletSigner).zapIn(vaultContract.address, 0, packedInput, packedConfig, r, s);
